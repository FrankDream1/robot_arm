...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)
                     ), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(
                c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
    ...
    def _inference(self, x):
        ...
        if self.export and self.format in {"tflite", "edgetpu"}:
            ...
            grid_size = torch.tensor(
                [grid_w, grid_h, grid_w, grid_h], device=box.device).reshape(1, 4, 1)
            ...
            dbox = self.decode_bboxes(
                self.dfl(box), self.anchors.unsqueeze(0)) * self.strides
...
class Pose(Detect):
    ...
    def __init__(self, nc=80, kpt_shape=(17, 3), ch=()):
        ...
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(
            c4, c4, 3), nn.Conv2d(c4, self.nk, 1)) for x in ch)

    def forward(self, x):
        """Perform forward pass through YOLO model and return predictions."""
        bs = x[0].shape[0]  # batch size
        kpt = torch.cat([self.cv4[i](x[i]).view(bs, self.nk, -1)
                        for i in range(self.nl)], -1)  # (bs, 17*3, h*w)
        x = Detect.forward(self, x)
        if self.training:
            return x, kpt
    ...
    def kpts_decode(self, bs, kpts):
        ...
        if self.export:
            if self.format in {
                ...
                grid_size = torch.tensor(
                    [grid_w, grid_h], device=y.device).reshape(1, 2, 1)
                norm = self.strides / (self.stride[0] * grid_size)
            ...
            y = kpts.clone()
            if ndim == 3:
                # sigmoid (WARNING: inplace .sigmoid_() Apple MPS bug)
                y[:, 2::3] = y[:, 2::3].sigmoid()
            y[:, 0::ndim] = (y[:, 0::ndim] * 2.0 +
                             (self.anchors[0] - 0.5)) * self.strides
            y[:, 1::ndim] = (y[:, 1::ndim] * 2.0 +
...
class WorldDetect(Detect):
    ...
    def __init__(self, nc=80, embed=512, with_bn=False, ch=()):
        ...
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(
            c3, c3, 3), nn.Conv2d(c3, embed, 1)) for x in ch)
        self.cv4 = nn.ModuleList(BNContrastiveHead(
            embed) if with_bn else ContrastiveHead() for _ in ch)

    def forward(self, x, text):
        """Concatenates and returns predicted bounding boxes and class probabilities."""
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv4[i]
                             (self.cv3[i](x[i]), text)), 1)
        if self.training:
            return x
...
class RTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.input_proj = nn.ModuleList(nn.Sequential(
            nn.Conv2d(x, hd, 1, bias=False), nn.BatchNorm2d(hd)) for x in ch)
    ...
    def forward(self, x, batch=None):
        ...
        # (bs, 300, 4+nc)
        y = torch.cat(
            (dec_bboxes.squeeze(0), dec_scores.squeeze(0).sigmoid()), -1)
        return y if self.export else (y, x)
    ...
    def _get_decoder_input(self, feats, shapes, dn_embed=None, dn_bbox=None):
        ...
        # Query selection
        # (bs, num_queries)
        topk_ind = torch.topk(enc_outputs_scores.max(-1).values,
                              self.num_queries, dim=1).indices.view(-1)
        # (bs, num_queries)
        batch_ind = torch.arange(
            end=bs, dtype=topk_ind.dtype).unsqueeze(-1).repeat(1, self.num_queries).view(-1)

        # (bs, num_queries, 256)
        top_k_features = features[batch_ind, topk_ind].view(
        ...
        embeddings = self.tgt_embed.weight.unsqueeze(0).repeat(
            bs, 1, 1) if self.learnt_init_query else top_k_features
        if self.training:
